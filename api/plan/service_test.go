    ...
    "plans": [
      {
		...
		plans, err := client.Plan().GetPlans()
		...
		assert.Equal(t, "TestPlan", b.Name)
...
func TestService_GetPlan(t *testing.T) {
  ...
  "data": {
    ...
    "server_knowledge": 473
		...
		client := ynab.NewClient("")
		_, err := client.Plan().GetPlan("aa248caa-eed7-4575-a990-717386438d2c", nil)
		assert.NoError(t, err)
	})
...
func TestService_GetLastUsedPlan(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
  ...
  "data": {
	...
	client := ynab.NewClient("")
	_, err := client.Plan().GetLastUsedPlan(nil)
	assert.NoError(t, err)
}

func TestService_GetPlanSettings(t *testing.T) {
	t.Run(`success`, func(t *testing.T) {
		httpmock.Activate()
		defer httpmock.DeactivateAndReset()
  ...
  "data": {
    "settings": {
      ...
      "currency_format": {
		...
		client := ynab.NewClient("")
		settings, err := client.Plan().GetPlanSettings("aa248caa-eed7-4575-a990-717386438d2c")
		assert.NoError(t, err)
		...
		expected := &plan.Settings{
			...
			CurrencyFormat: &plan.CurrencyFormat{
				ISOCode:          "BRL",
...
func TestService_GetPlansWithAccounts(t *testing.T) {
  ...
  "data": {
	...
	client := ynab.NewClient("")
	plans, err := client.Plan().GetPlansWithAccounts(true)
	assert.NoError(t, err)

	assert.Equal(t, 1, len(plans))
	assert.Equal(t, "aa248caa-eed7-4575-a990-717386438d2c", plans[0].ID)
	assert.Equal(t, "TestPlan", plans[0].Name)
}
